...
function EditarBlogPage() {
  ...
  const fetchPost = async () => {
    try {
      const response = await axios.get(`/cms/blog/${params.id}`);
      const post = response.data;
      
      setFormData({
        titulo: post.titulo,
        tituloSeo: post.tituloSeo || '',
        descripcion: post.descripcion,
        descripcionSeo: post.descripcionSeo || '',
      ...
      toast.error('Error', 'Error al cargar el artículo');
  ...
  const handleSubmit = async (e: React.FormEvent, estado?: typeof formData.estado) => {
    ...
    try {
      ...
      await axios.put(`/cms/blog/${params.id}`, dataToSend);
      toast.success('Artículo actualizado', estado === 'PUBLICADO' ? 'El artículo ha sido publicado exitosamente' : 'Los cambios han sido guardados');
      router.push('/cms/tienda/blog');
    } catch (error) {
      console.error('Error al actualizar artículo:', error);
      toast.error('Error', 'No se pudo actualizar el artículo. Por favor intenta de nuevo.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-4">
            ...
            >
              ...
              {loading ? 'Publicando...' : formData.estado === 'PUBLICADO' ? 'Actualizar Publicado' : 'Publicar Ahora'}
            </button>
          </div>
        </form>
      </div>
...
export default withCMSProtection(EditarBlogPage);
