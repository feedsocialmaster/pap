...
export default function ProductoClient({ producto }: { producto: Producto }) {
  ...
  useEffect(() => {
    if (hasVariants && colorSeleccionado && talleSeleccionado) {
      ...
      if (variant && cantidad > variant.stock) {
        ...
        // Si no hay variante para esta combinación, resetear cantidad
        setCantidad(0);
      }
    } else if (hasVariants) {
  ...
  const handleAgregarCarrito = () => {
    ...
    if (hasVariants && !colorSeleccionado) {
      toast.warning('Seleccioná un color', 'Por favor elegí un color antes de continuar');
      return;
    }

    // Si tiene variantes, validar que exista una variante para la combinación seleccionada
    if (hasVariants && !variantSeleccionada) {
      toast.error('Combinación no disponible', 'No hay stock para la combinación de color y talle seleccionada');
      return;
    }
                ...
                <p className="text-xs text-gray mt-2">
                  ...
                  {hasVariants && variantSeleccionada && talleSeleccionado && (
                    <span className="ml-2 font-semibold">
                      • Stock disponible (talle {talleSeleccionado}): {variantSeleccionada.stock} unidades
                    </span>
                  )}
                  {hasVariants && !talleSeleccionado && (
                    <span className="ml-2 text-yellow-600 dark:text-yellow-400">
                      • Selecciona un talle para ver el stock
                    </span>
                  )}
