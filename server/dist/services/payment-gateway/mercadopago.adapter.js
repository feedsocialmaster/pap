...
export class MercadoPagoAdapter {
    constructor() {
        this.sandboxUrl = 'https://api.mercadopago.com';
        this.productionUrl = 'https://api.mercadopago.com';
    }
    getBaseUrl(mode) {
    ...
    async createPayment(config, data) {
        try {
            ...
            const response = await axios.post(`${this.getBaseUrl('SANDBOX')}/checkout/preferences`, preference, {
                headers: {
                    Authorization: `Bearer ${config.apiKey}`,
            ...
            return {
                success: false,
                errorMessage: `Error creando pago en MercadoPago: ${error.response?.data?.message || error.message}`,
            };
        }
    }
