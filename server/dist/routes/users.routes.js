...
import { verifyEmailToken, resendVerificationEmail, } from '../services/email-verification.service.js';
 ...
 */
router.patch('/:id/telefono', requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
 ...
 */
router.patch('/:id/toggle-active', requireCMSAuth, async (req, res) => {
    try {
        const { id } = req.params;
