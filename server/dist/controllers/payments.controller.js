import { z } from 'zod';
import { createOrderAndPreference, handleWebhook, isPaymentServiceConfigured, handlePaymentSuccess, processPaymentApproval } from '../services/payment.service.js';
...
export async function createPreference(req, res) {
    try {
        ...
        console.log('User en request (JWT):', req.user);
...
export async function success(req, res) {
    ...
    // Procesar el pago aprobado y reducir stock
    if (orderId) {
        try {
            console.log(`🎉 [Success Callback] Procesando pago para orden ${orderId}`);
            await handlePaymentSuccess(orderId);
        }
        catch (error) {
            console.error(`❌ [Success Callback] Error procesando pago:`, error);
...
/**
 * Endpoint para aprobar un pago manualmente
 * Útil cuando el webhook de MercadoPago falla o en desarrollo local
 */
export async function approvePayment(req, res) {
    ...
    console.log(`🔧 [Manual Approval] Aprobando pago manualmente para orden ${orderId}`);
    try {
        ...
        res.json({
            ...
            message: result.processed
                ? 'Pago aprobado y stock reducido correctamente'
                : 'El pago ya había sido procesado anteriormente',
            order: result.order,
        });
    }
    catch (error) {
