...
export const getCategorias = async (req, res) => {
    try {
        const { pagina = '1', limite = '50', busqueda = '', activo, parentId, includeProducts = 'false' } = req.query;
...
export const crearCategoria = async (req, res) => {
    try {
        const { name, slug: customSlug, description, parentId, orden, activo, } = req.body;
        // Validaciones
        if (!name) {
...
export const getEstadisticas = async (req, res) => {
    try {
        const [totalCategorias, categoriasActivas, categoriasConProductos,] = await Promise.all([
...
export const asignarProductos = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const updateData = { categoryId: id };
        if (asPrimary) {
