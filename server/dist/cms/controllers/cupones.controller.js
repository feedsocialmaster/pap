...
async function verificarPromocionesExclusivas(productIds) {
    ...
    const promocionIds = [...new Set(productosConPromocion.map(p => p.tipoPromocionAplica).filter(Boolean))];
...
export const createCupon = async (req, res) => {
    try {
        const { codigo, descuento, tipoDescuento, tipoBundle, combinable, exclusivoConPromociones, activo, fechaInicio, fechaFin, usosMaximos, usuariosPermitidos, descripcion } = req.body;
...
export const verificarDisponibilidadCodigos = async (req, res) => {
    try {
