...
export const getPromociones = async (req, res) => {
    try {
        const { activo, destacado, vigente, pagina = '1', limite = '20' } = req.query;
...
export const crearPromocion = async (req, res) => {
    try {
        const { titulo, descripcion, imagenUrl, tipoDescuento, valorDescuento, leyendaPersonalizada, fechaInicio, fechaFin, activo, compraMinima, productosAplicables, tiposCalzadoAplicables, usosMaximos, usosPorUsuario, destacado, orden, colorFondo, colorTexto, exclusivaConCodigos, } = req.body;
        // Validaciones
...
export const getEstadisticas = async (req, res) => {
    try {
        const ahora = new Date();
        const [totalPromociones, promocionesActivas, promocionesVigentes, promocionesDestacadas, totalUsos,] = await Promise.all([
