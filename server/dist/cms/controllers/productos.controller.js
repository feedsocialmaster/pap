...
import { validateProduct, generateSlug, validateSlug } from '../../utils/product-validation.js';
...
export const getProductos = async (req, res) => {
    try {
        const { pagina = '1', limite = '20', busqueda = '', tipoCalzado, enLiquidacion, esFiesta, impermeable, antideslizante, ordenarPor = 'fechaCreacion', orden = 'desc' } = req.query;
...
export const crearProducto = async (req, res) => {
    try {
        const { nombre, descripcion, precio, tipoCalzado, imagenes, talles, colores, variants, // NEW: Array de variantes con stock por color
        stock, enLiquidacion, porcentajeDescuento, esFiesta, impermeable, antideslizante, caracteristicas, categoryId, slug: customSlug, metodosPago, retiroEnLocal, envioNacional, envioLocal, productoEnLanzamiento, } = req.body;
...
export const actualizarProducto = async (req, res) => {
    try {
        const { id } = req.params;
        const { nombre, descripcion, precio, tipoCalzado, imagenes, talles, colores, stock, enLiquidacion, porcentajeDescuento, esFiesta, impermeable, antideslizante, caracteristicas, categoryId, slug: customSlug, variants, // NEW: Array de variantes con stock por color
        metodosPago, retiroEnLocal, envioNacional, envioLocal, productoEnLanzamiento, aplicaPromocion, tipoPromocionAplica, } = req.body;
        ...
        if (hasVariants) {
            ...
            const stockTotalVariantes = variants.reduce((sum, v) => sum + (v.stock || 0), 0);
...
export const eliminarProducto = async (req, res) => {
    try {
        ...
        if (cantidadOrderItems > 0) {
            ...
            const ordenesActivas = producto.orderItems.filter(item => item.order.estado !== 'CANCELADO').length;
...
export const getEstadisticas = async (req, res) => {
    try {
        const [totalProductos, productosEnLiquidacion, productosSinStock, productosStockBajo, totalStock,] = await Promise.all([
...
// Validar slug único
export const validarSlug = async (req, res) => {
    try {
...
export const generarSlug = async (req, res) => {
    try {
...
export const getProductosConEstadisticas = async (req, res) => {
    try {
        ...
        // IMPORTANTE: Solo incluir orderItems de pedidos CONFIRMADOS
        ...
        const productosConEstadisticas = productos.map(producto => {
            // Calcular total de unidades vendidas (solo pedidos confirmados)
            const totalVendido = producto.orderItems.reduce((sum, item) => sum + item.cantidad, 0);
            // Calcular total de ventas en dinero (solo pedidos confirmados)
            const totalVentas = producto.orderItems.reduce((sum, item) => sum + (item.cantidad * item.precioUnitario), 0);
        ...
        res.json({
            ...
            resumen: {
                ...
                stockTotalGeneral: productos.reduce((sum, p) => sum + p.variants.reduce((s, v) => s + v.stock, 0), 0),
                unidadesVendidasTotal: productosConEstadisticas.reduce((sum, p) => sum + p.totalVendido, 0),
                ventasTotal: productosConEstadisticas.reduce((sum, p) => sum + p.totalVentas, 0)
            }
        });
    }
    catch (error) {
...
export const forceDeleteProducto = async (req, res) => {
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const ordenesAEliminar = new Set();
            const ordenesProtegidas = new Set();
...
async function deleteImagesFromStorage(imagePaths, correlationId) {
