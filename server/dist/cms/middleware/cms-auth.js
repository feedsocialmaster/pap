...
export const cmsAuth = (req, res, next) => {
    ...
    const cmsRoles = ['DUENA', 'DESARROLLADOR', 'GERENTE_COMERCIAL', 'ADMIN_CMS', 'SUPER_SU', 'VENDEDOR'];
    // Si el usuario no tiene un rol de CMS, denegar acceso
    if (!cmsRoles.includes(req.user.role)) {
...
export const requireCMSAccess = (allowedRoles) => {
    return (req, res, next) => {
