...
import { AuthedRequest, requireAuth, requireRole, Role } from '../../middleware/auth.js';
...
const SITE_CONTENT_ROLES: Role[] = ['DUENA', 'DESARROLLADOR', 'ADMIN_CMS', 'GERENTE_COMERCIAL', 'SUPER_SU'];
...
type ContentKey = typeof VALID_CONTENT_KEYS[number];

// Mapeo de keys a URLs públicas
const KEY_TO_URL: Record<ContentKey, string> = {
  'home_body': '/',
...
const KEY_TO_TITLE: Record<ContentKey, string> = {
  'home_body': 'Página Principal',
  'home_payment_promotions': 'Medios de Pago y Promociones',
  'privacy_policy': 'Política de Privacidad',
};
 ...
 */
function validateContentKey(req: AuthedRequest, res: Response, next: () => void) {
  ...
  if (!VALID_CONTENT_KEYS.includes(key as ContentKey)) {
 ...
 */
router.get('/', requireAuth, requireRole(...SITE_CONTENT_ROLES), async (req: AuthedRequest, res: Response) => {
  try {
    const locale = (req.query.locale as string) || 'es-AR';
 ...
 */
router.get('/:key', requireAuth, requireRole(...SITE_CONTENT_ROLES), validateContentKey, async (req: AuthedRequest, res: Response) => {
  try {
    const { key } = req.params;
    const locale = (req.query.locale as string) || 'es-AR';
    ...
    if (!content) {
      ...
      return res.json({
        ...
        content: {
          key,
          locale,
          title: KEY_TO_TITLE[key as ContentKey],
          content: '',
          version: 0,
          published: false,
          url: KEY_TO_URL[key as ContentKey],
        },
      });
    }
    ...
    res.json({
      ...
      content: {
        ...content,
        url: KEY_TO_URL[key as ContentKey],
        displayTitle: KEY_TO_TITLE[key as ContentKey],
      },
    });
  } catch (error) {
 ...
 */
router.post('/:key', requireAuth, requireRole(...SITE_CONTENT_ROLES), validateContentKey, async (req: AuthedRequest, res: Response) => {
  try {
    const { key } = req.params;
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const updated = await tx.siteContent.upsert({
        ...
        create: {
          key,
          locale,
          title: title || KEY_TO_TITLE[key as ContentKey],
 ...
 */
router.get('/:key/versions', requireAuth, requireRole(...SITE_CONTENT_ROLES), validateContentKey, async (req: AuthedRequest, res: Response) => {
  try {
    const { key } = req.params;
    const locale = (req.query.locale as string) || 'es-AR';
 ...
 */
router.get('/:key/versions/:versionNum', requireAuth, requireRole(...SITE_CONTENT_ROLES), validateContentKey, async (req: AuthedRequest, res: Response) => {
  try {
 ...
 */
router.post('/:key/rollback', requireAuth, requireRole(...SITE_CONTENT_ROLES), validateContentKey, async (req: AuthedRequest, res: Response) => {
  try {
    const { key } = req.params;
