 ...
 */
async function processPaymentNotification(externalId: string) {
  try {
    ...
    const result = await adapter.queryPayment((gateway as any).config, externalId);
 ...
 */
export async function genericWebhook(req: Request, res: Response, next: NextFunction) {
  try {
