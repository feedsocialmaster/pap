...
import {
  ...
  isFinalStatus,
} from '../lib/order-state-machine.js';
import {
  TIMESTAMP_FIELDS_BY_STATUS,
  STATUS_TO_ESTADO_ENTREGA,
} from '../types/order-status.js';
...
export class OrderManagementService {
   ...
   */
  async updateOrderStatus(params: UpdateOrderStatusParams) {
    const {
      orderId,
      newStatus,
      deliveryReason,
    ...
    const transitionResult = validateTransition(previousStatus, newStatus, {
      ...
      fulfillmentType: (order as any).fulfillmentType || 'shipping',
      intentosEntrega: (order as any).intentosEntrega || 0,
    ...
    if (timestampField) {
      dataToUpdate[timestampField as string] = now;
    }
    ...
    try {
      ...
      try {
        ...
        wsService.emitOrderStatusChanged(orderId, previousStatus, newStatus, {
          ...
          fulfillmentType: (result as any).fulfillmentType,
   ...
   */
  async getAvailableTransitionsForOrder(orderId: string) {
    const order = await prisma.order.findUnique({
      where: { id: orderId },
      select: {
        id: true,
