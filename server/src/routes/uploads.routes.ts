...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    if (!allowed.includes(file.mimetype)) return cb(new Error('Formato no soportado'));
    cb(null, true);
  },
});
...
const uploadVideo = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    if (!allowed.includes(file.mimetype)) return cb(new Error('Formato de video no soportado'));
    cb(null, true);
  },
});
